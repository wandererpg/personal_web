...
- Bot 管理入口与公开留言板，访客可发布文字和 PNG/JPEG/WebP 图片
- 单管理员登录，访客只能读取已发布文章
...
首页留言板为公开即时发布频道：每条留言最多 1000 字，最多上传 3 张图片，单张图片最大 4 MB，仅接受 PNG、JPEG 和 WebP。管理员可在 `/admin/guestbook` 删除留言；留言正文和图片保存在 `BLOG_DATA_DIR`，不进入 Git 仓库。
...
定期备份整个 `BLOG_DATA_DIR`，其中包含 `drafts/`、`media/`、`guestbook/`、`guestbook.json` 和 `sessions/`。推荐使用服务器快照或加密后的增量备份；恢复时先停止服务，恢复目录所有权，再启动服务。不要把该目录复制进公开仓库。
